<div x-data="{ open: false }"
    ...
    <button @click="open = !open" class="relative flex items-center gap-1.5 text-sm hover:text-amber-600 transition">
        <span class="relative inline-flex">
            ...
            @if($itemsCount > 0)
                <span class="absolute -top-1.5 -right-2 min-w-5 h-5 px-1 bg-amber-600 text-white text-xs rounded-full flex items-center justify-center font-medium leading-none">
                    {{ $itemsCount }}
                </span>
            @endif
        </span>
        <span class="hidden sm:inline">{{ __('shop.cart') }}</span>
    </button>
    ...
    <div x-show="open" x-transition
        ...
        @else
            <div class="text-sm text-gray-400 text-center py-3">{{ __('shop.cart_empty') }}</div>
        @endif
    </div>
</div>