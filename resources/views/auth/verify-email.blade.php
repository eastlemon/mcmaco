<x-guest-layout>
    ...
    <h1 class="text-3xl font-extrabold tracking-tight">{{ __('auth.verify_title') }}</h1>
    <p class="mt-1.5 text-sm text-gray-400">{{ __('auth.verify_hint') }}</p>
    ...
    <div class="mt-8">
        @if (session('status') == 'verification-link-sent')
            <div class="mb-6 flex items-start gap-3 rounded-xl bg-green-50 border border-green-100 p-4 text-sm text-green-700">
                ...
                <span>{{ __('auth.verify_sent') }}</span>
            </div>
        @endif

        <p class="text-sm text-gray-500 leading-relaxed">
            {{ __('auth.verify_spam_hint') }}
        </p>
