        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            <form method="POST" action="{{ $mode === 'create' ? route('ads.manage.store') : route('ads.manage.update', $ad) }}" class="bg-white shadow rounded-lg p-6 space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select name="condition" class="border rounded px-3 py-2 w-full">
                            <option value="new" @selected(old('condition', $ad->condition) === 'new')>Новое</option>
                            <option value="used" @selected(old('condition', $ad->condition) === 'used')>Б/у</option>
                        </select>
                ...
                <div class="flex justify-end">
                    <button class="bg-amber-600 text-white px-4 py-2 rounded">Сохранить</button>
                </div>
            </form>
            ...
            @if ($mode === 'edit')
                <div class="bg-white shadow rounded-lg p-6 mt-6" x-data="imageUploader({{ $ad->id }}, {{ $ad->images->count() }})">
                    <div class="flex items-center justify-between mb-4">
                        <h3 class="font-semibold">Фотографии (до 10)</h3>
                    ...
                    <div class="text-sm text-gray-500 mb-4">Добавляй по одной или несколько. Максимум 5MB на файл.</div>
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-5 gap-4">
                        @foreach($ad->images as $image)
                            <div class="border rounded p-2 text-center" x-data>
                                ...
                                <button class="mt-2 text-red-600 text-sm" @click="remove({{ $image->id }})">Удалить</button>
                            </div>
                        @endforeach
                    </div>
                </div>
            @else
                <div class="text-sm text-gray-500 mt-4">Фото можно добавить после сохранения объявления.</div>
            @endif
        </div>
    </div>
