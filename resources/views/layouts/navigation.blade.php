    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center h-16 gap-4">
            ...
            <div class="flex items-center gap-3">
                ...
                @else
                    <a href="{{ route('login') }}" class="text-sm text-gray-600 hover:text-gray-900 hidden sm:inline">Войти</a>
                    @if(Route::has('register'))
                        <a href="{{ route('register') }}" class="text-sm bg-amber-600 text-white px-3 py-1.5 rounded hover:bg-amber-700 hidden sm:inline">Регистрация</a>
                    @endif
                @endauth
    ...
    <div :class="{'block': open, 'hidden': !open}" class="hidden sm:hidden border-t border-gray-100">
        <div class="px-4 py-3 space-y-3">
            ...
            @auth
                ...
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit" class="text-sm text-gray-600 py-1">Выйти</button>
                </form>
            @else
                <a href="{{ route('login') }}" class="block text-sm text-gray-600 py-1">Войти</a>
                @if(Route::has('register'))
                    <a href="{{ route('register') }}" class="block text-sm text-amber-600 py-1">Регистрация</a>
                @endif
            @endauth
        </div>
    </div>
